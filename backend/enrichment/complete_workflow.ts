...
export const startCompleteEnrichment = api<
  StartCompleteEnrichmentRequest,
  CompleteEnrichmentResponse
>(
  ...
  async ({ filePath, fileName, serverId }) => {
    ...
    try {
      ...
      processCompleteEnrichmentWorkflow(
        jobId,
        filePath,
        fileName,
        serverId
...
async function processCompleteEnrichmentWorkflow(
  ...
  try {
    ...
    const { referenceFiles, blacklistFiles } = await extractAndSeparateFiles(
      processedFilePath,
      fileName
    );
    ...
    console.log(
      `Complete enrichment workflow finished successfully for job ${jobId}`
    );
  } catch (error: any) {
    console.error(
      `Error in complete enrichment workflow for job ${jobId}:`,
      error
    );
...
async function scanAndRetrieveFile(
  filePath: string,
  fileName: string
): Promise<string> {
  ...
  console.log(
    `File retrieved: ${fileName} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`
  );
  return filePath;
}

async function extractAndSeparateFiles(
  filePath: string,
  fileName: string
): Promise<{ referenceFiles: string[]; blacklistFiles: string[] }> {
  ...
  if (ext === ".zip") {
    ...
    const extractedDir = path.join(
      ...
      `${path.basename(fileName, ".zip")}_extracted`
    );
...
async function insertIntoTempTables(
  jobId: string,
  filePaths: string[]
): Promise<number> {
  let totalRecords = 0;
  ...
  for (const filePath of filePaths) {
    ...
    if (fileName.toLowerCase().startsWith("blacklist_mobile")) {
      console.log(
        `Detected blacklist file: ${fileName}, skipping temp insertion`
      );
      ...
      stream
        ...
        .on("data", async (row) => {
          try {
            ...
            const numeroClean = cleanString(numero);
            const voieClean = cleanString(voie);
            const villeClean = cleanString(ville);
            const codPostClean = cleanString(cod_post);

            const temp_hexacle_hash = [
              numeroClean || "",
              voieClean || "",
              villeClean || "",
              codPostClean || "",
            ].join("");
            const normalized_hash = temp_hexacle_hash
              .toUpperCase()
              .replace(/[\s\-\.]/g, "");
            ...
            const tempData = {
              job_id: jobId,
              file_name: fileName,
              temp_hexacle_hash: normalized_hash,
              hexacle_original: cleanString(hexacle),
              numero: numeroClean,
              voie: voieClean,
              ville: villeClean,
              cod_post: codPostClean,
    ...
    stream.on("data", (chunk) => {
      if (!hasReadFirstLine) {
        ...
        if (lineEnd !== -1) {
          ...
          const hasHeaders =
            firstLineClean.includes("HEXACLE") ||
  ...
  for (const record of batch) {
    try {
      await db.exec`
        ...
        VALUES (
          ${record.job_id}, ${record.file_name}, ${record.temp_hexacle_hash}, ${record.hexacle_original}, 
...
async function performEnrichment(
  jobId: string
): Promise<{ processed: number; matched: number; enriched: number }> {
  ...
  console.log(
    `Enrichment completed: ${processed} processed, ${matched} matched, ${enriched} enriched`
  );
  return { processed, matched, enriched };
}

async function applyBlacklistFiltering(
  jobId: string
): Promise<{ filtered: number; final: number }> {
  ...
  // On vérifie mobile et phone des données enrichies (selon la structure de la table contacts)
  await db.exec`
    DELETE FROM enrichment_results 
    WHERE job_id = ${jobId} 
    AND found_match = true 
    AND (
      -- Vérifier mobile (sans le 0 initial)
      SUBSTRING(enriched_data->>'mobile' FROM 2) IN (
        SELECT phone_number FROM mobile_blacklist
      )
      OR
      -- Vérifier phone (sans le 0 initial)  
      SUBSTRING(enriched_data->>'phone' FROM 2) IN (
        SELECT phone_number FROM mobile_blacklist
      )
      OR
      -- Vérifier mobile avec le 0
      enriched_data->>'mobile' IN (
        SELECT '0' || phone_number FROM mobile_blacklist
      )
      OR
      -- Vérifier phone avec le 0
      enriched_data->>'phone' IN (
        SELECT '0' || phone_number FROM mobile_blacklist
      )
    )
  `;
  ...
  console.log(
    `Mobile blacklist filtering completed: ${filtered} filtered out, ${final} final records`
  );
  return { filtered, final };
}

async function generateFinalResultZip(
  jobId: string,
  ...
  const csvFilePath = path.join(
    resultsDir,
    `enriched_${originalFileName.replace(".zip", ".csv")}`
  );
  ...
  for (const result of results) {
    ...
    const enrichedData = result.found_match
      ? JSON.parse(result.enriched_data)
      : {};
    ...
    const row = [
      ...
      refData.cod_post || "",
      refData.cod_insee || "",
      enrichedData.prenom || "", // prenom au lieu de first_name
      enrichedData.nom || "", // nom au lieu de last_name
      enrichedData.email || "",
      enrichedData.mobile || "", // mobile au lieu de mobile_phone
      enrichedData.phone || "", // phone au lieu de landline_phone
      enrichedData.adresse || "", // adresse au lieu de address
      enrichedData.ville || "", // ville au lieu de city
      enrichedData.code_postal || "", // code_postal au lieu de postal_code
    ];
